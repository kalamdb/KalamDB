---
name: Shared table RLS
overview: PostgreSQL-shaped shared-table RLS compiled to an authorization IR, then executed with the cheapest safe strategy—PointGuard/MultiGuard, DataFusion 55 LeftSemi/HashJoin, or a generation-aware authorization cache. RLS never runs policy SQL per row or per live event. Identity is bound after the user-independent plan cache. Mutable-column RLS applies only after MVCC winner selection.
todos:
  - id: rls-catalog-model
    content: Persist TablePolicy definitions (not execution plans); system.table_policies; policy/schema generations; reverse dependency registry; DROP TABLE cleanup
    status: pending
  - id: rls-ddl
    content: Parse CREATE/ALTER/DROP POLICY; PostgreSQL-shaped permissive syntax; reject RESTRICTIVE and ACCESS_LEVEL
    status: completed
  - id: rls-access-migration
    content: ACCESS_LEVEL is not SQL. Shared tables are FORCE RLS default-deny; grants are CREATE POLICY only
    status: completed
  - id: rls-compiler
    content: Compile USING/WITH CHECK to PolicyProgram IR (RowLocal, AuthorizationRelation); no snapshot_sql; dependency DAG and invalidation metadata
    status: pending
  - id: rls-strategy
    content: Select PointGuard, MultiGuard, DataFusionSemiJoin, or CachedAuthorizationSet from bound policy plus safe client predicates; no hardcoded IN vs HashSet thresholds
    status: pending
  - id: rls-query-barrier
    content: Bind principal after plan-cache lookup; post-MVCC RLS barrier; late-materialize RLS keys; reuse DF 55 LeftSemi/HashJoin dynamic filters for broad membership
    status: pending
  - id: rls-indexes
    content: Reuse IndexedEntityStore for PointGuard/prefix membership lookups; warn or require covering (principal, relation_key) indexes; do not invent a parallel RLS index system
    status: pending
  - id: rls-dml
    content: One evaluator for INSERT/UPDATE/DELETE/ON CONFLICT/RETURNING/batch/transaction paths; reject unsupported write paths rather than bypass
    status: pending
  - id: rls-auth-cache
    content: Cache Arc AuthorizationSet by policy+principal+dependency generation; commit-ordered invalidation; fail closed on stale positive auth
    status: pending
  - id: rls-live
    content: Bind evaluator at subscribe; 0 SQL/0 DF in fan-out; PointGuard for equality subscriptions; inverted auth index optional; old/new visibility transitions
    status: pending
  - id: rls-side-channels
    content: Deny User/Service raw shared file download; audit ON CONFLICT, exports, ingestion, maintenance, backup
    status: pending
  - id: rls-observability
    content: EXPLAIN strategy (PointGuard vs DF semi-join); DF/Kalam metrics; missing-index warnings; never print another principal's keys
    status: pending
  - id: rls-tests
    content: Security matrix, MVCC winner vs RLS, PointGuard, plan-cache isolation, live grant/revoke e2e, ON CONFLICT; DF EXPLAIN fixtures; performance benchmarks
    status: pending
  - id: rls-docs
    content: SQL reference, RLS vs WHERE, index guidance, migration, live semantics, kalamdb-skills mirrors
    status: pending
isProject: false
---

# Shared-table RLS (performance rewrite)
...
Checkout fact: workspace pins **Apache DataFusion 55.0.0** ([Cargo.toml](Cargo.toml) `datafusion = { version = "55.0.0", ... }`, [Cargo.lock](Cargo.lock) `checksum = 96f76f0167ed0842b29a3d1e41be3c034c0a46409a3c703cc4cc84ee8c24abf4`). APIs below were verified against `~/.cargo/registry/.../datafusion-*-55.0.0` and `datafusion-physical-plan-55.0.0`, not from memory.
...
| Capability | Where | Default | Plan implication |
|---|---|---|---|
| `EXISTS` / `IN` subquery → `LeftSemi` / `LeftAnti` | `datafusion-optimizer-55.0.0` `DecorrelatePredicateSubquery` | on | Broad membership SQL should be a semi-join, not a custom nested loop |
| `HashJoinExec` + `DynamicFilterPhysicalExpr` | `datafusion-physical-plan-55.0.0` `joins/hash_join/exec.rs` | on | Build side becomes a dynamic probe filter |
| Adaptive IN vs hash membership | `PushdownStrategy::{InList, Map}` | `hash_join_inlist_pushdown_max_size=128KiB`, `hash_join_inlist_pushdown_max_distinct_values=150` | **Do not** write Kalam `if n < N { IN } else { HashSet }` on the SQL join path |
| Join/TopK/Agg dynamic filter pushdown | `enable_dynamic_filter_pushdown`, `enable_join_dynamic_filter_pushdown` | **true** | Keep enabled; do not disable for RLS |
| Parquet bloom + page-index pruning | Kalam already sets `with_parquet_bloom_filter_pruning(true)` and `with_parquet_page_index_pruning(true)` in [`datafusion_session.rs`](backend/crates/kalamdb-core/src/sql/datafusion_session.rs) | on | Dynamic IN filters can prune cold segments |

Kalam [`SessionConfig`](backend/crates/kalamdb-core/src/sql/datafusion_session.rs) does **not** override the hash-join IN thresholds; DF 55 defaults apply. First EXPLAIN/benchmark task: confirm a `LeftSemi` on `group_members` actually attaches `PushdownStrategy` to the `group_messages` scan.
...
- [`kalamdb-datafusion-sources`](backend/crates/kalamdb-datafusion-sources/src/lib.rs): shared `ExecutionPlan` scaffolding, pruning descriptors, deferred MVCC exec.
- [`mvcc_filter_evaluation`](backend/crates/kalamdb-datafusion-sources/src/pruning.rs): **exact** filters run on **resolved winners**; **inexact** pre-winner pruning is only PK equality, `_seq` bounds, `_deleted`. Mutable `group_id IN (...)` is **not** in that inexact set today — keep it that way.
- [`compute_metadata_only_cold_columns`](backend/crates/kalamdb-tables/src/utils/base.rs) / `compute_cold_columns`: already metadata-first / projection-aware cold reads.
- [`IndexedEntityStore` + `IndexDefinition`](backend/crates/kalamdb-store/src/indexed_store.rs): prefix `scan_by_index`, `find_best_index_for_filters`. Compound example: [`JobStatusCreatedAtIndex`](backend/crates/kalamdb-system/src/providers/jobs/jobs_indexes.rs) `(status, created_at, job_id)`.
- Shared user tables today: [`SharedTablePkIndex`](backend/crates/kalamdb-tables/src/shared_tables/pk_index.rs) only `(pk, seq)`. There is **no** general `CREATE INDEX` for `(user_id, group_id)` on shared tables (dialect `CREATE INDEX` is vector cosine). PointGuard is **not** O(1) unless we cover the membership lookup with PK/index/`AuthorizationSet`.
- Plan cache: [`PlanCacheKey`](backend/crates/kalamdb-core/src/sql/executor/sql_executor.rs) = namespace + role + SQL. User-agnostic on purpose.
- Live: [`compile_subscription_filter`](backend/crates/kalamdb-live/src/manager/queries_manager.rs) → [`RowFilter`](backend/crates/kalamdb-row-filter); fan-out [`dispatch_chunk`](backend/crates/kalamdb-live/src/notification.rs). Subqueries rejected. Catch-up uses SQL/`scan`.
- DML: [`transaction_batch_insert.rs`](backend/crates/kalamdb-core/src/sql/executor/transaction_batch_insert.rs) ON CONFLICT paths.

---

## Product model (unchanged intent)
...
2. Membership relation unique/PK covering `(user_id, group_id)` → [`SharedTablePkIndex`](backend/crates/kalamdb-tables/src/shared_tables/pk_index.rs) / `IndexedEntityStore` point lookup, then MVCC-resolve **that** membership row.
...
Reuse [`IndexedEntityStore`](backend/crates/kalamdb-store/src/indexed_store.rs).
...
Same evaluator: `visible_old_row` / `check_new_row`. Defaults (`DEFAULT CURRENT_USER()`) before WITH CHECK. Batch atomic. ON CONFLICT INSERT vs UPDATE branches both go through it ([`transaction_batch_insert.rs`](backend/crates/kalamdb-core/src/sql/executor/transaction_batch_insert.rs)). Unsupported path → error, not bypass.
...
Subscribe: bind `BoundPolicy` + share `Arc<AuthorizationSet>` / PointGuard result on [`SubscriptionHandle`](backend/crates/kalamdb-live/src/models/connection.rs).
...
Idempotent migration to `_kdb_migrated_public_read` / `_kdb_migrated_service_all` as previously specified. New tables default-deny.
...
Keep the previous security matrix, plus:
...
- **DF fixture:** `EXPLAIN` of injected SemiJoin contains `HashJoinExec` / `LeftSemi` (lock the shape).
- **CLI e2e:** group chat, websocket grant/revoke, `OR 1=1`, ON CONFLICT hidden PK, raw download deny ([`smoke_test_shared_table_subscription.rs`](cli/kalam-cli-e2e/tests/smoke/subscription/smoke_test_shared_table_subscription.rs) sibling).
