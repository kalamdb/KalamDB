//! Built-in Dart/Flutter project templates bundled with the CLI.
...
pub fn available() -> Vec<&'static EmbeddedTemplate> {
    templates_for_language("dart")
}

pub fn resolve(template_id: Option<&str>) -> Result<&'static EmbeddedTemplate> {
    if let Some(template_id) = template_id {
        find_template("dart", template_id).ok_or_else(|| {
            CLIError::ConfigurationError(format!("unknown dart template '{template_id}'"))
        })
    } else {
        default_template_for_language("dart")
    }
}
