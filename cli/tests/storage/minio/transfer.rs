...
use crate::common::{
    ...
    is_server_running, leader_or_server_url, wait_for_job_finished,
};
...
fn test_minio_shared_table_export_import_roundtrip() {
    ...
    let export_response = start_table_export_sync(&token, &namespace, &source_table, "shared", None)
    ...
    let download_url = if download_url.starts_with("http://") || download_url.starts_with("https://") {
        download_url.to_string()
    } else {
        format!("{}{}", leader_or_server_url().trim_end_matches('/'), download_url)
    };

    let (status_code, content_type, zip_bytes) = http_get_with_token(&download_url, &token)
        .expect("download table export zip");
    ...
    let hot_row_count = query_count(&format!(
        "SELECT COUNT(*) AS c FROM {} WHERE body = 'hot-zeta'",
        target_full
    ));
    ...
    let source_dir = resolve_template(&storage_meta.shared_template, &namespace, &source_table, None);
    let target_dir = resolve_template(&storage_meta.shared_template, &namespace, &target_table, None);
