...
pub fn workspace_root() -> PathBuf {
    ...
    loop {
        if path.join("backend").join("Cargo.toml").exists() && path.join("cli").exists() {
            return path;
        }
        if !path.pop() {
            break;
...
fn execute_sql_via_cli_as_with_args_and_urls(
    ...
    for attempt in 0..max_attempts {
        ...
        for (idx, url) in urls.iter().enumerate() {
            ...
            let mut child = Command::new(kalam_bin());
            child
                .arg("-u")
                .arg(url)
                .arg("--user")
        ...
        let result: Result<kalam_client::QueryResponse, Box<dyn std::error::Error + Send + Sync>> =
            async {
                ...
                ) -> Result<kalam_client::QueryResponse, Box<dyn std::error::Error + Send + Sync>>
                {
                    ...
                    let query_params = params.as_ref().map(|values| {
                        kalam_client::query::models::query_param::params_from_json_values(
                            values.clone(),
                        )
                    });
...
pub fn create_cli_command() -> assert_cmd::Command {
    ...
    let mut cmd = assert_cmd::Command::new(kalam_bin());
...
pub fn get_storage_dir() -> std::path::PathBuf {
    ...
    let backend_path = workspace_root().join("backend/data/storage");
    if backend_path.exists() {
        return backend_path;
    }

    // Fallback for different working directory contexts (legacy root path)
    let root_path = workspace_root().join("data/storage");
    if root_path.exists() {
        return root_path;
    }
