...
impl FunctionActivation {
    ...
    /// Refuse activation when ABI, contract hash, or export set do not match.
    pub fn validate_activation(
        ...
        if artifact_abi != ABI_VERSION {
            return Err(FunctionsError::AbiMismatch {
                artifact: artifact_abi,
                runtime:  ABI_VERSION,
            });
        }
        if artifact_contract_hash != catalog_contract_hash {
            return Err(FunctionsError::ContractMismatch(format!(
                "artifact {artifact_contract_hash}, catalog {catalog_contract_hash}"
            )));
        }
        if !exports_match {
            return Err(FunctionsError::ContractMismatch(
                "module export set does not match catalog".into(),
            ));
        }
        Ok(())
    }
    ...
    pub async fn load_revision(
        ...
        let source = storage
            ...
            .await
            .map_err(|error| FunctionsError::Storage(error.to_string()))?;
        let source = String::from_utf8(source.data.to_vec())
            .map_err(|error| FunctionsError::Invalid(format!("artifact is not utf8: {error}")))?;
        Ok(ModuleRevision {
    ...
    pub fn rollback(
        ...
        Self::validate_activation(artifact_abi, &contract_hash, catalog_contract_hash, true)?;
        let artifact = self
            .stores
...
mod tests {
    ...
    #[test]
    fn activation_mismatch_keeps_caller_from_swapping() {
        let err = FunctionActivation::validate_activation(1, "a", "a", true).unwrap_err();
        assert!(matches!(
            err,
        ...
        let err = FunctionActivation::validate_activation(ABI_VERSION, "a", "b", true).unwrap_err();
        assert!(matches!(err, FunctionsError::ContractMismatch(_)));
        let err =
            FunctionActivation::validate_activation(ABI_VERSION, "a", "a", false).unwrap_err();
        assert!(matches!(err, FunctionsError::ContractMismatch(_)));
        FunctionActivation::validate_activation(ABI_VERSION, "ok", "ok", true).unwrap();
    }

    #[tokio::test]
    #[ntest::timeout(30000)]
    async fn rollback_rejects_missing_artifact_and_bad_abi() {
        let storage = StorageCached::with_default_timeouts(test_storage());
        let activation =
            FunctionActivation::new(CatalogStores::new(Arc::new(InMemoryBackend::new())));
        let module_id = FunctionModuleId::new("backend");
        ...
        let err = activation
            ...
            .unwrap_err();
        assert!(err.to_string().contains("missing function artifact"), "{err}");
        ...
        let err = activation
            ...
            .unwrap_err();
        assert!(matches!(err, FunctionsError::AbiMismatch { .. }));
        let still = activation.active_module(&FunctionModuleId::new("backend")).unwrap().unwrap();
        assert_eq!(still.contract_hash.as_deref(), Some("contract-v1"));
    }
}
