...
mod tests {
    ...
    use crate::FunctionsError;

    #[test]
    fn scope_timeout_covers_nested_children() {
        let cancel = CancellationToken::new();
        let root = crate::InvocationScope {
            deadline: Instant::now() + Duration::from_millis(1),
            cancel:   cancel.clone(),
            depth:    0,
        };
        std::thread::sleep(Duration::from_millis(3));
        let child = crate::InvocationScope {
            deadline: root.deadline,
            cancel:   root.cancel.clone(),
            depth:    1,
        };
        assert!(matches!(child.check(), Err(FunctionsError::Timeout)));
        cancel.cancel();
        let queued = crate::InvocationScope {
            deadline: Instant::now() + Duration::from_secs(5),
            cancel,
            depth: 0,
        };
        assert!(matches!(queued.check(), Err(FunctionsError::Cancelled)));
    }
}
