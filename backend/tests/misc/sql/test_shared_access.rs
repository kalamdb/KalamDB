...
async fn test_access_level_is_rejected() {
    ...
    let create_table_sql = format!(
        "CREATE TABLE {}.messages (id BIGINT PRIMARY KEY, content TEXT) WITH (TYPE = 'SHARED', ACCESS_LEVEL = 'PUBLIC')",
        namespace
    );
...
async fn test_omitted_policy_is_dba_system_only() {
    ...
    let create_table_sql = format!(
        "CREATE TABLE {}.docs (id BIGINT PRIMARY KEY, content TEXT NOT NULL) WITH (TYPE = 'SHARED')",
        namespace
    );
    ...
    assert!(user_select.rows_as_maps().is_empty(), "user should see zero rows without a policy");
...
async fn test_select_policy_grants_user_reads_not_writes() {
    ...
    let create_table_sql = format!(
        "CREATE TABLE {}.announcements (id BIGINT PRIMARY KEY, message TEXT NOT NULL) WITH (TYPE = 'SHARED')",
        namespace
    );
    assert_eq!(
        server
            .execute_sql_as_user(&create_table_sql, service_id.as_str())
            .await
            .status,
        ResponseStatus::Success
    );
    ...
    let select = server
        .execute_sql_as_user(&format!("SELECT * FROM {}.announcements", namespace), user_id.as_str())
        .await;
...
async fn test_public_all_policy_allows_user_writes() {
    ...
    let create_table_sql = format!(
        "CREATE TABLE {table} (id BIGINT PRIMARY KEY, content TEXT NOT NULL) WITH (TYPE = 'SHARED')"
    );
    assert_eq!(
