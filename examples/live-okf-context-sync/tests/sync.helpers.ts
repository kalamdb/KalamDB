...
export async function stopSyncApp(app: FolderSyncApp | undefined): Promise<void> {
  if (!app) {
    return;
  }
  try {
    ...
    // Best-effort cleanup so open live subscriptions do not keep the test runner alive.
  }
}
