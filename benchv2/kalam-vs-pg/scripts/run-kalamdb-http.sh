#!/usr/bin/env bash
# Run the KalamDB HTTP SQL column (POST /v1/api/sql, hot-only).
set -euo pipefail
...
KALAMDB_PGWIRE_PORT="$PG_PORT" \
    "$BIN" "$SETUP/server.toml" >"$RESULTS/kalamdb-http-server.log" 2>&1 &
SERVER_PID=$!
...
cd "$ROOT"
cargo build --release --bin kalam_vs_pg_kalamdb_http
OUT="$RESULTS/kalamdb-http-$(date +%Y%m%d-%H%M%S).txt"
{
  echo "# KalamDB vs PostgreSQL — KalamDB (HTTP /v1/api/sql, hot-only)"
  echo "# server_bin=${BIN}"
  echo "# http=127.0.0.1:${HTTP_PORT}"
  echo "# protocol=POST /v1/api/sql parameterized; pool=reqwest idle=64; concurrency=16"
  echo "# http2_prior_knowledge=${KALAMDB_HTTP2:-0}"
  echo "# timed_read_response=bytes; HTTP status validation only"
  echo "# flush.check_interval_seconds=0; table created without FLUSH_POLICY"
  echo "# rocksdb.memory_mode=server (cache≈RAM/8)"
  echo "# durability=sync_writes=false, WAL on"
  echo "# started=$(date -u +%Y-%m-%dT%H:%M:%SZ)"
  KALAMDB_URL="http://127.0.0.1:${HTTP_PORT}" \
    ./target/release/kalam_vs_pg_kalamdb_http
