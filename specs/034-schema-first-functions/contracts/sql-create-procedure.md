...
CREATE parses the wrapped JavaScript in V8 and rejects host-API misuse (`ctx.log(...)` instead of `ctx.log.info(...)`, calling `ctx.db` as a function). Isolate `console.log` is valid and forwards to the process logger. CREATE does not invoke the procedure, so it will not catch logic errors or missing arguments. Success reports `created` vs `replaced` plus the inline artifact id. It does not create a function module revision.
