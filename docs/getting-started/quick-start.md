...
Run a realtime React chat app with a topic-trigger procedure inside KalamDB, then open it in two tabs to see changes arrive live. The reply is simulated, so you do not need an external AI key.
...
`kalam init` downloads the chat starter and installs its dependencies. `kalam dev` starts or reuses a local KalamDB server, applies `kalam/schema.sql`, generates TypeScript tables, activates schema-first procedures, and runs the browser app.
...
You should see the message in both tabs, live copilot progress, and a saved reply. The reply is deterministic demo logic; you can replace it with your own model call in `functions/src/chat_demo/on_user_message.ts`.
...
- Read `kalam/schema.sql` for shared rooms, the personal USER inbox, STREAM progress events, procedures, and the topic trigger.
- Edit `functions/src/chat_demo/on_user_message.ts` to change how new messages are answered.
- Keep `kalam dev` running to apply schema changes, regenerate types, and activate procedures as you work. The demo schema includes reset statements; inspect it before adapting it to data you want to keep.
