...
> Revised 2026-09-01. This replaces the earlier design that spread FlatBuffers/FlexBuffers logic across model, storage, Raft, and stream modules.

## 1. Goal

KalamDB must have exactly one internal serialization subsystem for database objects.
...
2. **One storage-object envelope**
   - Every structured object persisted by KalamDB uses the same versioned Kalam Object Protocol envelope.
   - Rows, metadata objects, topic envelopes, Raft durable objects, and stream records all enter through this protocol.
...
5. **Storage boundaries serialize once**
   - Serialization happens immediately before persistence and deserialization happens immediately after reading persisted bytes.
   - No publisher/service should pre-serialize an object only for the store to serialize or wrap it again.

6. **Hot rows remain optimized**
   - Centralization does not mean forcing every object through one slow generic representation.
   - `kalamdb-serialization` may internally have optimized payload profiles, but all profiles share one public API, envelope, versioning, errors, metrics, and compatibility policy.

7. **External protocols stay external**
   - HTTP JSON, WebSocket JSON/MessagePack, PGWire, export metadata, SDK wire formats, and user topic payload bytes are not database-object persistence codecs.
...
The current code already contains multiple independent persistence serialization paths.
...
- `kalamdb-commons` therefore contains persistence implementation rather than only shared models/types.

Target:

- Move all persistence codec code, generated FlatBuffer code, schema files, version constants, and compatibility logic to `kalamdb-serialization`.
...
That creates two places that must be updated whenever a new DataFusion/KalamDB value type is added.
...
- `SharedTableRow` mixes the row model and store implementation in the same file.
...
- `EntityStore` should accept serde-capable generic objects and use the generic object serializer by default.
...
- This raw-key exception must be explicit and documented; it must not become a way to bypass object serialization for arbitrary values.
...
- a custom FlexBuffers command envelope with its own `v` and `kind` fields;
...
- Keep only command/domain type definitions and, where useful, stable semantic command-kind identifiers.
- Encode/decode commands, responses, durable Raft metadata, state-machine payloads, and snapshots that KalamDB owns through `kalamdb-serialization`.
- OpenRaft/tonic/protobuf network transport may remain separate because it is an external library/network protocol boundary, not KalamDB RocksDB object persistence.
...
- The length-prefixed record frame should also be represented by a small centralized helper so the stream file format has one implementation and version policy.
...
Current publisher code already contains a TODO about redundant topic-message serialization and manually pre-encodes `TopicMessage` before giving raw bytes to the store.

Target:

- Publisher creates `TopicMessage` objects and payload bytes only.
...
- Remove public/internal APIs that exist only to pass pre-encoded database objects between KalamDB modules.
...
- System metadata is lower-volume and should use the centralized generic object profile so adding a field does not require changing Rust + a second hand-maintained FlatBuffer model schema.
...
kalamdb-commons          Domain IDs, rows, data types, shared models
       ^
       |
kalamdb-serialization    The only persisted-object codec implementation
       ^
       |
...
Callers should see a small API. Exact Rust names can change during implementation, but the architectural boundary should look like this:
...
`EncodedObject` should expose bytes and encoded length without causing a second serialization:
...
For batch hot paths, add buffer-reuse APIs inside the same crate rather than duplicating serialization in callers:
...
impl ObjectEncoder {
    ...
    pub fn encode_user_row(&mut self, row: &UserTableRow, schema: &StorageSchema)
        -> Result<EncodedObject>;
}
```

The important rule is that the caller chooses **what semantic object it is encoding**, not the binary library used to encode it.
...
The exact FlatBuffer schema may use equivalent fields, but these semantics must remain stable.

### Why keep `object_kind` but hide `codec_kind`

Application code needs to validate that a row is a row and a Raft command is the expected protocol kind. Application code does not need to know whether the payload was produced by FlatBuffers, FlexBuffers, or a future codec.

If codec migration requires old and new codecs to coexist, codec/version dispatch belongs entirely inside the serialization crate. A private codec ID can be stored in the envelope if needed, but it is not exposed as a choice to callers.

### Version rules

1. `protocol_version` changes only when the common envelope or global decoding contract changes.
2. `schema_version` represents the logical payload/storage schema version where required.
...
5. No callsite performs its own version check.

## 7. Internal Payload Profiles

Centralization should not sacrifice storage efficiency. The crate can use two internal profiles initially.
...
The selected library is private. If a benchmark later shows another generic object codec is better, only `kalamdb-serialization::object` changes.

### 7.2 Table Row Profile

Use for USER and SHARED table rows because they dominate object count and storage volume.

Initial codec: FlatBuffers with one generic row schema.

The row codec is schema-aware and optimized around table/type metadata rather than repeating self-describing names for every value.
...
The current row FlatBuffer uses `ColumnValue { name, value }`. The target row layout must be a true ordinal vector.
...
Column names live once in table schema metadata, not once in every RocksDB value.

### 8.2 Do not repeat type information that the schema already knows

For a normal table column, the table schema already says whether physical slot 0 is `BIGINT`, slot 1 is `TEXT`, slot 2 is `STRUCT`, etc.
...
- keep explicit tags only where the value is genuinely dynamic, such as arbitrary JSON/dynamic content.

This reduces both RocksDB value size and CPU spent repeatedly decoding metadata that is already known.

### 8.3 Physical storage ordinals are stable

Schema evolution must not make old row bytes ambiguous.
...
6. Persist a storage `schema_version` with the row so compatibility/default behavior can be applied centrally.

This is similar in spirit to append-only binary schema evolution and prevents a full RocksDB rewrite for ordinary additive schema changes.

### 8.4 Nested `STRUCT` / `CREATE TYPE` values

The storage scalar codec must support recursive values in one place.
...
Storage representation uses the type schema's stable field slots recursively:
...
The same central scalar/value implementation must cover:
...
No second struct/list/map serializer may be added to table stores, DataFusion sources, function runtime, or SDK-facing code.
...
The decoder sees the older `schema_version` / shorter ordinal vector and materializes the missing field according to centralized evolution rules:

- nullable additive field -> `NULL`;
- field with a safe database default -> apply the defined read/migration semantics;
...
Raw primary-key references in an index may remain raw ordered/reference bytes where that is the most efficient representation. They are not considered database-object payloads.
...
4. Remove persistence-only FlatBuffers/FlexBuffers dependencies from commons/store/tables/system/raft/streams/vector as those callsites disappear.
5. Keep MessagePack/JSON dependencies where they are genuinely required by external WebSocket/API/export contracts.
...
7. Move storage-specific scalar wire-tag mapping out of `KalamDataType` if those tags are only used by persistence; keep logical SQL type identity in commons.
...
6. Do not add codec-specific helper functions to `kalamdb-store`.

### 10.4 `kalamdb-tables`

Changes:

1. User/shared row stores call central row encoder/decoder.
...
4. Remove APIs that accept pre-serialized database objects unless they are explicitly raw/opaque payload APIs.
...
3. Vector index engine/file formats are independent and do not become part of the Kalam Object Protocol unless they store KalamDB structured objects in RocksDB.

### 10.8 `kalamdb-raft`

Changes:

1. Replace direct FlexBuffers use in `src/codec/command_codec.rs` with centralized protocol encode/decode calls, then delete/reduce the file once no logic remains.
2. Replace MessagePack helpers in `src/state_machine/serde_helpers.rs` with centralized serialization calls and remove the helper module when possible.
3. Command kind/version validation belongs in the centralized protocol implementation.
4. Durable Raft state stored in RocksDB uses the common object envelope.
5. Tonic/protobuf/OpenRaft network RPC serialization remains a network transport concern and is out of scope unless KalamDB itself persists those encoded frames.
...
2. be documented as an explicit non-object exception such as ordered key bytes or opaque user payload bytes.
...
- SDK wire formats;
- transfer/export manifest JSON intended as a public/portable format;
...
This boundary prevents accidental coupling of user-facing protocols to RocksDB's internal storage format.
...
5. Persisted Raft data must remain decodable for the supported upgrade window.
...
Storage/Raft/stream layers convert this error into their own domain error only at the boundary.

Do not construct codec-specific error strings independently across callers.

## 14. Legacy Data Policy

For this migration, keep the existing decision not to perform silent automatic historical rewrites.
...
3. Do not guess between multiple codecs at every read forever.
...
5. Once the supported legacy window expires, remove the legacy decoder from the serialization crate without touching callers.
...
At minimum, inspect server persistence paths for forbidden direct usage:
...
This turns "serialization lives in one place" into an architectural invariant rather than a convention that can slowly regress.
...
Do not duplicate this matrix in each table/store crate.
...
- Raft proposal persistence where applicable.

## 17. Performance Gates

Centralization is accepted only if it stays efficient.
...
- target improvement from removing repeated column names should be reported;
- memory allocations per row should not increase.
...
- representative AI/chat JSON-like payload stored in a typed struct.
...
2. Classify each match as:
   ...
   - test only.
3. Record baseline payload size, encode/decode latency, and batch throughput.
...
No caller should directly select a codec after this phase.
...
2. Convert the row format from name-keyed columns to real schema-ordinal values.
...
4. Ensure row stores use the optimized row profile while generic EntityStore objects use the generic profile.
...
1. Replace the custom FlexBuffers command codec with centralized protocol encoding.
2. Replace MessagePack state-machine helpers.
3. Move Kalam-owned durable Raft values to the common object envelope.
...
6. Document dev reset workflow for unsupported legacy data.
...
4. Nested objects (`STRUCT` / named `CREATE TYPE`) are encoded recursively by that same row/value codec.
...
9. Topic persistence serializes each message object once.
...
The intended architecture is deliberately simpler: **one crate, one envelope, one compatibility policy, one scalar/row implementation, and no persistence codec logic scattered through the database.**
