  ...
  group('runBridgeSubscriptionStream', () {
    test('delivers decoded events and closes the handle when exhausted',
      ...
      final stream = runBridgeSubscriptionStream<Object, String, String>(
        ...
        cancel: () async {},
        decode: (_, value) => value.toUpperCase(),
        isDisposed: () => false,
      );
    ...
    test('waits for consumer progress before pulling the next event', () async {
      final source = Queue<String>.from(['alpha', 'beta']);
      final firstReceived = Completer<void>();
      final releaseProgress = Completer<void>();
      final received = <String>[];
      final stream = runBridgeSubscriptionStream<Object, String, String>(
        description: 'backpressured-stream',
        prepare: () async {},
        open: () async => Object(),
        next: (_) async => source.isEmpty ? null : source.removeFirst(),
        close: (_) async {},
        cancel: () async {},
        decode: (_, value) => value,
        waitBeforeNext: (value) =>
            value == 'alpha' ? releaseProgress.future : Future.value(),
        isDisposed: () => false,
      );

      final completed = stream.listen((value) {
        received.add(value);
        if (value == 'alpha') firstReceived.complete();
      }).asFuture<void>();
      await firstReceived.future;
      await Future<void>.delayed(Duration.zero);

      expect(received, ['alpha']);
      expect(source, ['beta']);
