...
class KalamClient {
  ...
  Stream<ChangeEvent> liveEvents(
    ...
    return runBridgeSubscriptionStream<bridge.DartLiveEventsSubscription,
      ...
      decode: (_, event) => _fromBridgeChangeEvent(
        event,
  ...
  /// Open live changes whose reconnect cursor advances only after the caller
  /// acknowledges an event. Use this for durable local synchronization.
  Stream<LiveEventDelivery> liveEventsWithAck(
    String sql, {
    int? batchSize,
    int? lastRows,
    SeqId? from,
    ...
    return runBridgeSubscriptionStream<bridge.DartLiveEventsSubscription,
      ...
      decode: (subscription, event) {
        final checkpoint = _checkpointFromBridgeChangeEvent(event);
        return LiveEventDelivery(
          event: _fromBridgeChangeEvent(event, onError: onError),
          checkpoint: checkpoint,
          acknowledge: () => checkpoint == null
              ? Future.value()
      ...
      waitBeforeNext: (delivery) => delivery.checkpoint == null
          ? Future.value()
  ...
  Stream<List<T>> live<T>(
    ...
    return runBridgeSubscriptionStream<bridge.DartLiveRowsSubscription,
      ...
      decode: (_, event) => _fromBridgeLiveRowsEvent(
        event,
        mapRow: mapRow,
        onCheckpoint: onCheckpoint,
      ),
