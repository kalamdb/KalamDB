...
typedef BridgeSubscriptionDecode<THandle, TBridgeEvent, TEvent> = TEvent
    Function(
  THandle handle,
  TBridgeEvent event,
);
typedef BridgeSubscriptionWait<TEvent> = Future<void> Function(TEvent event);
...
Stream<TEvent> runBridgeSubscriptionStream<THandle, TBridgeEvent, TEvent>({
  ...
  required BridgeSubscriptionDecode<THandle, TBridgeEvent, TEvent> decode,
  BridgeSubscriptionWait<TEvent>? waitBeforeNext,
  ...
  Future<void> runLoop() async {
    ...
    try {
      ...
      while (!closed && !isDisposed()) {
        ...
        final decoded = decode(openedHandle, event);
        controller.add(decoded);
        if (waitBeforeNext != null) {
          await Future.any([waitBeforeNext(decoded), cancelled.future]);
        }
      }
