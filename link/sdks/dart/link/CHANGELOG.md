...
### Added
- `KalamClient.exchangeOidcToken()` exchanges a Keycloak/OIDC ID token for KalamDB JWTs (`POST /v1/api/auth/oidc/exchange-token`).
- Typed procedure clients (`KalamFunctions`) are generated by `kalam schema gen --languages dart`, not by a generic `KalamClient.call()`.
