...
CLI_ENV_FILE="$PROJECT_ROOT/cli/.env"
...
check_node_sql_auth_ready() {
    local base_url=$1
    get_access_token "$base_url" >/dev/null 2>&1
}

wait_for_cluster_sql_auth_ready() {
    local ready=0

    for _i in {1..60}; do
        local count=0
        check_node_sql_auth_ready "http://127.0.0.1:$NODE1_HTTP" && ((count++)) || true
...
start_cluster() {
    ...
    if [ "$healthy" -eq 1 ]; then
        ...
        echo -e "${YELLOW}Waiting for SQL/auth readiness on all nodes...${NC}"
        if ! wait_for_cluster_sql_auth_ready; then
            echo -e "${RED}Cluster HTTP endpoints are up, but SQL/auth readiness did not converge on all nodes.${NC}"
            echo "Check logs with: ./scripts/cluster.sh logs 1"
            exit 1
        fi
        ...
        echo "  cargo nextest run -p kalam-cli-e2e --test e2e cluster"
    else
...
write_cli_cluster_env() {
    cat > "$CLI_ENV_FILE" << EOF
# KalamDB Test Configuration
# Auto-generated by scripts/cluster.sh for local 3-node cluster testing.
...
EOF
    ...
    echo -e "${GREEN}✓ Updated $CLI_ENV_FILE for local cluster testing${NC}"
}
...
run_cluster_tests() {
    ...
    RUST_TEST_THREADS=1 \
        cargo test -p kalam-cli-e2e --test e2e cluster -- --nocapture
}
...
run_smoke_tests() {
    ...
    KALAMDB_ADMIN_PASSWORD="$ADMIN_PASSWORD" \
    RUST_TEST_THREADS=1 \
        cargo test -p kalam-cli-e2e --test e2e smoke -- --nocapture
}
    ...
    for node_url in "${nodes[@]}"; do
        ...
        if KALAMDB_SERVER_URL="$node_url" \
           ...
           KALAMDB_ROOT_PASSWORD="$ROOT_PASSWORD" \
              KALAMDB_ADMIN_PASSWORD="$ADMIN_PASSWORD" \
           RUST_TEST_THREADS=1 \
           cargo test -p kalam-cli-e2e --test e2e smoke_test_core_operations -- --nocapture; then
...
run_verification_tests() {
    ...
    KALAMDB_ADMIN_PASSWORD="$ADMIN_PASSWORD" \
    RUST_TEST_THREADS=1 \
        cargo test -p kalam-cli-e2e --test e2e cluster_test_system_tables -- --nocapture
    ...
    KALAMDB_ADMIN_PASSWORD="$ADMIN_PASSWORD" \
    RUST_TEST_THREADS=1 \
        cargo test -p kalam-cli-e2e --test e2e cluster_test_subscription -- --nocapture
    ...
    KALAMDB_ADMIN_PASSWORD="$ADMIN_PASSWORD" \
    RUST_TEST_THREADS=1 \
        cargo test -p kalam-cli-e2e --test e2e cluster_test_table_identity -- --nocapture
    ...
    KALAMDB_ADMIN_PASSWORD="$ADMIN_PASSWORD" \
    RUST_TEST_THREADS=1 \
        cargo test -p kalam-cli-e2e --test e2e cluster_test_final -- --nocapture
...
run_workspace_nextest_e2e() {
    print_header
    echo -e "${YELLOW}Preparing fresh local cluster for CLI nextest e2e run...${NC}"
    echo ""

    stop_cluster >/dev/null 2>&1 || true

    if [ -d "$CLUSTER_DATA_DIR" ]; then
        echo -e "${YELLOW}Removing existing local cluster state...${NC}"
        rm -rf "$CLUSTER_DATA_DIR"
    fi
...
show_help() {
    ...
    echo "  workspace-e2e Clean-start local cluster, rewrite cli/.env, and run CLI nextest"
    echo "                (includes dedicated cluster tests already)"
    ...
    echo "  $0 workspace-e2e            # Fresh cluster + cli/.env rewrite + CLI nextest"
    echo ""
}
...
if [ "$MODE" = "docker" ]; then
    ...
    case "${1:-}" in
        ...
        help|--help|-h|"")
            show_help
            ;;
        smoke|smoke-all|verify|full|workspace-e2e)
